...
def check_crossover(df):
    ...
    is_above = (latest_row['EMA_10']
                > latest_row['EMA_50']) and (latest_row['EMA_20']
                                             > latest_row['EMA_50'])
...
def handle_start_command(chat_id):
    ...
    keyboard = {
        "inline_keyboard": [
            ...
            [{
                "text": "LIST1 - 15M",
                "callback_data": "/EMA LIST1 15M"
            }, {
                "text": "LIST1 - 1H",
                "callback_data": "/EMA LIST1 1H"
            }, {
                "text": "LIST1 - 4H",
                "callback_data": "/EMA LIST1 4H"
            }, {
                "text": "LIST1 - D",
                "callback_data": "/EMA LIST1 D"
            }, {
                "text": "LIST1 - W",
                "callback_data": "/EMA LIST1 W"
            }],

            # Buttons for LIST2
            [{
                "text": "LIST2 - 15M",
                "callback_data": "/EMA LIST2 15M"
            }, {
                "text": "LIST2 - 1H",
                "callback_data": "/EMA LIST2 1H"
            }, {
                "text": "LIST2 - 4H",
                "callback_data": "/EMA LIST2 4H"
            }, {
                "text": "LIST2 - D",
                "callback_data": "/EMA LIST2 D"
            }, {
                "text": "LIST2 - W",
                "callback_data": "/EMA LIST2 W"
            }],
        ]
    }
...
def check_coin_availability(symbol, timeframe):
    try:
...
def analyze_list(list_name, timeframe, chat_id):
    try:
        if list_name not in SYMBOL_LISTS:
            raise ValueError(
                f"Invalid list name. Available lists: {', '.join(SYMBOL_LISTS.keys())}."
            )

        # Add diagnostic check before analysis
        problematic_coins = []
        for symbol in SYMBOL_LISTS[list_name]:
            is_available, error = check_coin_availability(symbol, timeframe)
            if not is_available:
                problematic_coins.append(f"{symbol} (Error: {error})")

        if problematic_coins:
            error_message = "⚠️ Warning: The following coins have issues:\n" + "\n".join(
                problematic_coins)
            send_telegram_message(chat_id, error_message)
    ...
    while True:
        try:
            ...
            for update in updates:
                ...
                if "message" in update and update["message"].get(
                        "text") == "/start":
                ...
                if "message" in update and update["message"].get(
                    ...
                    try:
                        ...
                        if len(parts) != 3:
                            raise ValueError(
                                "Invalid command format. Use /EMA LISTNAME TIMEFRAME or /EMA SYMBOL TIMEFRAME."
                            )

                        target = parts[1].upper(
                        )  # Extract list name or symbol
                        timeframe = parts[2].upper(
                        )  # Extract timeframe (e.g., 15M)

                        # Check if the target is a list or a symbol
                        if target in SYMBOL_LISTS:
                            analyze_list(target, timeframe,
                                         update["message"]["chat"]["id"])
                        else:
                            analyze_symbol(target, timeframe,
                                           update["message"]["chat"]["id"])
                        ...
                        send_telegram_message(update["message"]["chat"]["id"],
                                              error_message)
